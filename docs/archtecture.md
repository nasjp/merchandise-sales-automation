...
* **DB infra/local stack**: `Supabase CLI`
...
`drizzle.config.ts` はここに置き、migration 出力先だけ `../../supabase/migrations` に向けます。
...
* test（backend test）:
  ...
  * migration を毎回同じ `supabase/migrations` から適用できる helper を置く
...
* **SQL migration 正本**: `supabase/migrations`

### 実務ルール

* Drizzle schema から migration を生成
* 出力先は `supabase/migrations`
* RLS / extension / trigger / view / manual SQL は hand-written migration として同じ `supabase/migrations` に置く
* `supabase/seed.sql` は最低限の初期 target と role だけ
...
この切り方にする理由は、**アプリ実装の型安全は Drizzle**、**インフラ反映と local stack は Supabase CLI**、という役割分離ができるからです。Supabase の exposed schema は RLS 前提なので、`raw_events` や `ai_runs` のような内部テーブルは private schema に寄せ、UI に見せる必要があるものだけ server 経由で出す設計にした方が安全です。([Supabase][3])
...
[3]: https://supabase.com/docs/guides/database/drizzle "https://supabase.com/docs/guides/database/drizzle"
